 function [ out, state ] = controller2( in, state )
...
[~,TrackingIndex1] = sort(PossibleMoves1);
if TrackingIndex1(1)==1
	newPosition1.x = in.x1;
    newPosition1.y = in.y1;
elseif TrackingIndex1(1)==2
	newPosition1.x = in.x2;
    newPosition1.y = in.y2;
elseif TrackingIndex1(1)==3
...
if (~isempty(in.m))
 if (~((in.x==in.xd)&&(in.y==in.yd)))
    ...
    [~,TrackingIndex2] = sort(PossibleMoves2);
    if TrackingIndex2(1)==1
        newPosition2.x = in.m.x1;
        newPosition2.y = in.m.y1;
    elseif TrackingIndex2(1)==2
        newPosition2.x = in.m.x2;
        newPosition2.y = in.m.y2;
    elseif TrackingIndex2(1)==3
    ...
    i = 0;
    while(1)          
%         PossibleMoves1(TrackingIndex1(1)) = [];
%         [BestMoveValue1,BestMoveIndex1] = min(PossibleMoves1);          
        i = i +1;
        if TrackingIndex1(i)==1
            newPosition1.x = in.x1;
            newPosition1.y = in.y1;
        elseif TrackingIndex1(i)==2
            newPosition1.x = in.x2;
            newPosition1.y = in.y2; 
        elseif TrackingIndex1(i)==3
            newPosition1.x = in.x3;
            newPosition1.y = in.y3;
        end
        if(~((newPosition1.x==in.m.x && newPosition1.y==in.m.y && newPosition2.x==in.x && newPosition2.y==in.y)||(newPosition1.x==newPosition2.x  && newPosition1.y==newPosition2.y)...
           ||((in.xd==in.x&&in.x==newPosition1.x)&&(abs(in.yd-in.y)<abs(in.yd-newPosition1.y)))...
